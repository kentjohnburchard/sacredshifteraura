...
import { Activity, Eye, Zap, Target, Users, Settings, Store, Code, BarChart, Shield, Gauge, ShieldAlert, Lock, Code as Nodes } from 'lucide-react';
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<'orchestration' | 'consciousness' | 'events' | 'circle' | 'toggle' | 'marketplace' | 'developer' | 'analytics' | 'integrity' | 'metrics' | 'unity-engine'>('orchestration');
  ...
  const [unityEngineComponent, setUnityEngineComponent] = useState<React.ComponentType | null>(null);
  ...
  useEffect(() => {
    if (!isUserMode && (activeTab === 'circle' || activeTab === 'events' || activeTab === 'marketplace' || activeTab === 'developer' || activeTab === 'analytics' || activeTab === 'unity-engine')) {
      const loadComponent = async () => {
        ...
        try {
          if (activeTab === 'circle') {
            ...
            const { ModuleMarketplace } = await import('./marketplace');
            setMarketplaceComponent(() => ModuleMarketplace);
          } else if (activeTab === 'developer') {
            // Import the developer component
            const { ModuleDeveloperKit } = await import('./developer');
            setDeveloperComponent(() => ModuleDeveloperKit);
          } else if (activeTab === 'analytics') {
            // Import the analytics component
            const { ModuleAnalytics } = await import('./developer');
            setAnalyticsComponent(() => ModuleAnalytics);
          } else if (activeTab === 'unity-engine') {
            // Load the Unity Engine component
            const module = await moduleManager.ensureModuleWithCapability('unity-web');
            if (module) {
              const exposedItems = module.getExposedItems();
              if (exposedItems.Component && typeof exposedItems.Component === 'function') {
                const ComponentFunction = exposedItems.Component();
                setUnityEngineComponent(() => ComponentFunction);
              }
            }
          }
        } catch (error) {
                ...
                {isUserMode ? (
                  ...
                  <main className="container mx-auto px-6 py-8">
                    ...
                    <div className="space-y-8">
                      ...
                      {activeTab === 'unity-engine' && (
                        <div>
                          {isLoadingComponents ? (
                            <div className="bg-slate-900/50 rounded-xl border border-purple-500/20 p-8 text-center">
                              <div className="w-16 h-16 border-4 border-purple-400/30 border-t-purple-400 rounded-full animate-spin mx-auto mb-4"></div>
                              <p className="text-purple-300">Loading Unity Engine...</p>
                            </div>
                          ) : unityEngineComponent ? (
                            React.createElement(unityEngineComponent)
                          ) : (
                            renderModuleLockedFallback('Unity Engine')
                          )}
                        </div>
                      )}
